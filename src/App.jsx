...
const translations = {
  ja: {
    title: "行列演算トレーナー",
    subtitle: "(行列の掛け算)", // "(成分, 足し算, 引き算, スカラー倍, 掛け算, 行列式, 逆行列)",
    newProblem: "新しい問題を作る",
    checkAnswer: "答え合わせ",
    hideAnswer: "答えを隠す",
    problem: "問題",
  ...
  en: {
    ...
    subtitle: "(Matrix multiplication)", // "(Entry, Addition, Subtraction, Scalar multiplication, Matrix multiplication, Determinant, Inverse matrix)",
...
export default function App() {
  ...
  // 問題種別
  const [mode, setMode] = useState("cdot");
  // 行列
  const [op, setOp] = useState("cdot");
  ...
  const newProblem = (overrideMode = null) => {
    ...
    // 1) まず演算を選ぶ
    const ops = ["cdot"]; // ["entry", "+", "-", "times", "cdot", "det", "inv", "row"];
